...
The bridge binds to `127.0.0.1:8789`. Do not expose it through a Kubernetes Service, Ingress, host port, or public Docker port. Set a high-entropy `CPA_SUBSCRIPTION_BRIDGE_SECRET` in both the CPA container and bridge sidecar.
...
- GitHub Copilot requests use the official `github-copilot-sdk` and its pinned, checksum-verified runtime download.
