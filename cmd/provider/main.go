...
func callHost(method string, payload any) (json.RawMessage, error) {
	...
	if len(rawPayload) > 0 {
		requestPointer = (*C.uint8_t)(C.CBytes(rawPayload))
		defer C.free(unsafe.Pointer(requestPointer))
	}
	code := C.call_host_api(cMethod, requestPointer, C.size_t(len(rawPayload)), &response)
...
func cliproxyPluginCall(method *C.char, request *C.uint8_t, requestLen C.size_t, response *C.cliproxy_buffer) C.int {
	...
	if method == nil {
		writeResponse(response, failure("invalid_method", "method is required"))
		return 1
	}
